...
type InferWidgetUnion<T extends WidgetType> = T extends keyof typeof WidgetType ? WidgetItem<T> : never;

export type WidgetUnion = InferWidgetUnion<WidgetType>;

type WidgetContentType<T extends WidgetContentMap, K extends WidgetType> = K extends keyof T ? T[K] : never;
...
export class WidgetItem<T extends WidgetType, C extends WidgetContentType<WidgetContentMap, T> = WidgetContentType<WidgetContentMap, T>> {
  content: C;
  type: T;

  constructor(type: T, content: ValidateShape<C, WidgetContentType<WidgetContentMap, T>>) {
    this.type = type;
