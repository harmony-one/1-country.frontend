...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (owner !== deployer) {
    const c = await ethers.getContract("RegistrarController", deployer);
    const tx = await c.transferOwnership(owner);
    console.log(
      `Transferring ownership of RegistrarController to ${owner} (tx: ${tx.hash})...`
    );
    await tx.wait();
  }
  ...
  const artifact = await deployments.getArtifact("IRegistrarController");
  ...
  console.log(
    `Setting RegistrarController interface ID ${interfaceId} on .eth resolver (tx: ${tx3.hash})...`
  );
  await tx3.wait();
};
